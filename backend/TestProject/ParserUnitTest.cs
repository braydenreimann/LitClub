...
namespace TestProject //dotnet test
{
    ...
    public class ParserUnitTest
    {
        ...
        public void BookParseTest()
        {
            ...
            Assert.AreEqual("Unbroken", books[22].Title); //Final Book in sample csv
            Assert.AreEqual("Laura Hillenbrand", books[22].Author);
            Assert.AreEqual(473, books[22].Editions[0].PrintLength);
            Assert.AreEqual(BookFormat.Hardcover, books[22].Editions[0].Format);
            Assert.AreEqual("978-1400064168", books[22].Editions[0].Isbn13s[0]);
